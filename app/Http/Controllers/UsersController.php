...
class UsersController extends RentcarController
{
	...
	public function show($username)
    {
        ...
        if( $validator->fails() == false ){
            $model = Admin_user::where('username', $username)->firstOrFail();
    ...
    public function update(Request $r)
    {
        ...
        $validator->after(function($validator) use ($input) {
            ...
            $user = Admin_user::where('username', Auth::user()->username)->first();
        ...
        if( $validator->fails() == false ){
	        ...
	        $model = Admin_user::where('username', Auth::user()->username)->firstOrFail();
    ...
    public function reset_password(Request $r)
    {
        ...
        $validator->after(function($validator) use ($input) {
            ...
            $user = Admin_user::where('phone', $input['phone'])->count();
            //dd($user->username);
            if ($user == 0){
        ...
        if( $validator->fails() == false ){
            //mencari data di table Admin_user.
            $user = Admin_user::where('phone', $r->input('phone'))->first();
    ...
    public function request_forgot_password(Request $r)
    {
        ...
        $validator->after(function($validator) use ($input) {
            ...
            $user = Admin_user::where('email', $input['email'])->count();
            //dd($user->username);
            if ($user == 0){
        ...
        if( $validator->fails() == false ){
            //mencari data di table Admin_user.
            $user = Admin_user::where('email', $r->input('email'))->first();
    ...
    public function forgot_password(Request $r, $secretcode)
    {
        ...
        $validator->after(function($validator) use ($input) {
           ...
           $user_count = Admin_user::where('secretcode', $input['secretcode'])->count();
            //dd($user_count);
            if ($user_count == 0){
        ...
        if( $validator->fails() == false ){
            ...
            $user = Admin_user::where('secretcode', $secretcode)->first();
    ...
    public function request_phone(Request $r)
    {
        ...
        if( $validator->fails() == false ){
            //mencari data di table Admin_user.
            $user = Admin_user::where('username', $r->input('username'))->first();
            //1. mengirim SMS ke usernya.
            $nohp  = $user->phone;
    ...
    public function postRegister(Request $request)
    {
        ...
        if( $validator->fails() == false ){
            ...
            $user->image = 'http://192.168.1.4/valagoo-admin/public/image/rentcar.jpg';
