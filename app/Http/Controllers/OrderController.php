...
class OrderController extends RentcarController
{
    ...
    /*public function store(Request $r)
    {
        ...
        $validator->after(function($validator) use ($input) {
            ...
            if ($service_count == 0) {
                ...
                //validasi 2. end_date harus sesuai dengan jumlah days (end_date tidak boleh melebihi days).
                $date1 = $input['order_date'];
                ...
                $days_max = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24)) + 1;

                if ($days_max > $input['days']) {
                    $validator->errors()->add('days', 'End Date Tidak Bisa Melebihi Days');
                }

                //validasi 3. end_date tidak boleh kurang dari order date
                if (strtotime($input['end_date']) < strtotime($input['order_date'])) {
                    $validator->errors()->add('end_date', 'End Date Tidak Boleh Kurang Dari Order Date');
                }
            }
        });
        ...
        if( $validator->fails() == false ){
            ...
            $model->end_date      = date('Y-m-d', strtotime($r->input('end_date')));
    ...
    /*public function uploadImage(Request $request)
    {
        ...
        if( $validator->fails() == false ){
            ...
            if($success){
                $order = Order::where('user', Auth::user()->username)->first();
                $order->image = $newName;
                $order->save();

                $ket = $this->responsejson('Image Berhasil Di Upload', 200, true, $order->user);
            }else{
